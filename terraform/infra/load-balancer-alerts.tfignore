...
resource "azurerm_monitor_metric_alert" "wowza_lb_alert" {
  ...
  action {
    action_group_id = azurerm_monitor_action_group.lb_action_group[each.key].id
  }
}
...
resource "azurerm_automation_webhook" "webhook" {
  ...
  enabled                 = true
  runbook_name            = module.dynatrace_runbook[0].runbook_name
...
resource "azurerm_monitor_action_group" "lb_action_group" {
  ...
  name                = "dynatrace-${each.value.lb_name}"
